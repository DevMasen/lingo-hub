...
function PaymentStatus() {
  ...
  const [query] = useSearchParams(); // 'status' 'message'
  ...
  return (
    ...
    >
      <div
        className={`relative w-[400px] flex-col items-center space-y-3 rounded-lg bg-opacity-65 px-12 py-8 ${(status === null || status === 'filed') && 'bg-red-700'} ${!isOpen ? 'hidden' : 'flex'}`}
      >
        <progress
        ...
        />
        <h2
          className={`text-center text-2xl font-semibold ${(status === null || status === 'failed') && 'text-red-200'}`}
        >
