...
- For normal development work, prefer creating a local branch in the existing repository checkout with `git switch -c <branch-name>` instead of creating a new Git worktree.
- Only create a Git worktree when the user explicitly asks for isolated parallel work or a specific worktree-based workflow.
