...
- `src/api/services/*.service.js` — **only** files allowed to import the Supabase client or call `.from(...)`. One file per resource: `auth.service.js`, `rooms.service.js`, `reservations.service.js`, `profiles.service.js`, `news.service.js`. Every exported function:
...
- `src/api/mappers/*.mapper.js` — snake_case Supabase row ⇄ camelCase domain object conversion (`toX`, `toXs`, `toXRow`). This is the *only* place that should need to change when swapping backends.
- `src/api/types/*.types.js` — JSDoc `@typedef`s documenting each domain shape (Room, Reservation, Profile, NewsItem). Non-executable; would become real `.ts` interfaces if the project ever adopts TypeScript.

**When migrating to Django:** only the *bodies* of service functions change (swap `supabase.from(...)` for `fetch('/api/...')`); mappers may need field-name tweaks; everything above (hooks, components) should not need to change.
...
⚠️ React Query is a declared dependency and `QueryClientProvider` wraps the app, but per project memory it was recently *unused* in places — check before assuming every data need already has a hook.

### Feature folder layout

`src/features/<domain>/` groups components + hooks + (sometimes) Context for one domain: `authentication`, `reserve`, `wallet`, `setting`, `header`, `sidebar`, `dashboard`. `src/ui/` holds generic, domain-agnostic building blocks (`Modal`, `Skeleton`, `Spinner`, `PanelButton`, `HomeButton`, etc.).
